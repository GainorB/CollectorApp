...
class Collection extends Component {
  ...
  componentDidMount() {
    if (this.props.user) {
      ...
      console.log('No userid for collection');
    }

    const sneakers = ReactDOM.findDOMNode(this.refs.sneakers);
    dragula([sneakers]);
  }
    ...
    if (this.props.apiDataLoaded === true) {
              ...
              <div className="boxbox" key={index}>
                ...
                <div className="hidden">
                  ...
                  <span className="cInfo">
                    <p>
                      Condition: {element.condition}/<strong>10</strong>
                    </p>
                    <p>
                      Size: <strong>{element.size}</strong>
                    </p>

                    {element.purchasedfor === 0
                      ? <p />
                      : <p>
                          Purchased For:{' '}
                          <strong>${element.purchasedfor}</strong>
                        </p>}
                    {element.purchasedfrom === ''
                      ? <p />
                      : <p>
                          Purchased From:{' '}
                          <strong>{element.purchasedfrom}</strong>
                        </p>}
                    {element.worth === 0
                      ? <p />
                      : <p>
                          Worth? <strong>${element.worth}</strong>
                        </p>}
                    {element.forsale === ''
                      ? <p />
                      : <p>
                          For Sale? <strong>{element.forsale}</strong>
                        </p>}
                    <br />
                    <p>
                  ...
                  <Link
                    ...
                    to="/update"
                    onClick={() => this.handleUpdate(element.id)}
                  >
                    Update {element.brand} {element.title}?
                  </Link>
                </div>
              </div>
            );
      ...
      return (
        <div className="loading">
          <img
            src="https://s-media-cache-ak0.pinimg.com/originals/a3/dd/08/a3dd083f4e57a8629e1e9de3a9f5a630.gif"
            alt="Loading"
          />
        </div>
      );
